function [LocoModel, BPMData, CMData, FitParameters, LocoFlags, RINGData] = locoTH(LocoMeasData, BPMData, CMData, FitParameters, LocoFlags, RINGData, metaData)
...
% Initialize
RMSGoal = 1e-7;           % RMS change in the response matrix for parameter changes [meters]
RMSToleranceFactor = 1E3;  % Lower tolerance for excepting the response matrix without recomputing (/3 for upper tolerance)
...
[A, b_old, Mmodel, iNoCoupling, LocoFlags] = BuildTheJacobian(LocoMeasData, LocoModel, BPMData, CMData, CMDataRM, FitParameters, LocoFlags, RINGData, A_NRows, A_NCols, N, Kicks, BPMIndexShort, RMSGoal, RMSToleranceFactor,metaData);
...
function [A, b_old, Mmodel, iNoCoupling, LocoFlags] = BuildTheJacobian(LocoMeasData, LocoModel, BPMData, CMData, CMDataRM, FitParameters, LocoFlags, RINGData, A_NRows, A_NCols, N, Kicks, BPMIndexShort, RMSGoal, RMSToleranceFactor,metaData)
...
if strcmpi((LocoFlags.Dispersion),'yes')
    ...
    Mmodel = locoresponsematrixFull(RINGData, BPMData, CMDataRM, LocoFlags, 'RF', FitParameters.DeltaRF);
else
    Mmodel = locoresponsematrixFull(RINGData, BPMData, CMDataRM, LocoFlags);
end
fprintf('%f seconds. \n',toc);


% % Find nominal response matrix
% fprintf('   Loading nominal response matrix (%s, %s) ... ', LocoFlags.ResponseMatrixCalculator, LocoFlags.ClosedOrbitType); 
% tmp = load(sprintf('%sRM%05d_disp%d_coupl%d',metaData.RMfolder,0,metaData.disp,metaData.coupl));
% Mmodel = tmp.Mmodel;
...
if strcmpi((CMData.FitCoupling),'yes')
    ...
    if strcmpi((LocoFlags.Dispersion),'yes')
        % Add "dispersion" as a column of the response matrix
        GR = locoresponsematrixFull(RINGData, BPMData, CMDataRM, LocoFlags, 'RF', FitParameters.DeltaRF);
    else
        GR = locoresponsematrixFull(RINGData, BPMData, CMDataRM, LocoFlags);
    end
    fprintf('%f seconds. \n', toc);
...
tmpA = zeros(size(A,1),length(FitParameters.Params));
% tmpA = cell(length(FitParameters.Params),1);



% Chose from file or calculate live
if metaData.usePreCalc
	for i = 1:length(FitParameters.Params)
		
		fprintf('Loading RM for parameter %d from file %s\n',i,sprintf('%sRM%05d_disp%d_coupl%d',metaData.RMfolder,FitParameters.Params{i}.ElemIndex,metaData.disp,metaData.coupl))
		tmp = load(sprintf('%sRM%05d_disp%d_coupl%d',metaData.RMfolder,FitParameters.Params{i}.ElemIndex,metaData.disp,metaData.coupl));
	...
	parfor (i = 1:length(FitParameters.Params), parforArg)
		tmpRINGData=[];Mdiff=[];Mparam=[];RMSDelta=[];
		
		
		% Compute the response matrix with the parameter change
	tmpRINGData = locosetlatticeparam(RINGData, FitParameters.Params{i}, FitParameters.Values(i)+FitParameters.Deltas(i));
	
	fprintf('   Parameter #%d, Computing response matrix (%s, %s) ... ', i, LocoFlags.ResponseMatrixCalculator, LocoFlags.ClosedOrbitType); tic
	if strcmpi((LocoFlags.Dispersion),'yes')
		% Add "dispersion" as a column of the response matrix
			Mparam = locoresponsematrixFull(tmpRINGData, BPMData, CMDataRM, LocoFlags, 'RF', FitParameters.DeltaRF);
		else
			Mparam = locoresponsematrixFull(tmpRINGData, BPMData, CMDataRM, LocoFlags);
		end
		fprintf('%f seconds. \n', toc);
		...
		tmpA(:,i) = Mdiff(iNoCoupling)/FitParameters.Deltas(i);
	end
end
A(:,(Acolindex):end) = tmpA;
b_old = [b_old; FitParameters.Values];
